...
Example with resourceRules and without `objectResource: true`.

```yaml
  - name: deny-privilege-escalationper-resource-enforce
    policyName: "pss-privilege-escalation.vap-library.com"
    validationActions: "[Deny]"
    namespaceLabel: security/policy
    namespaceSelector: enforce
    resourceRules:
      - apiGroups: [""]
        apiVersions: ["v1"]
